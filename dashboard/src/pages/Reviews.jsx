...
import {
  ...
  Star,
  Lock,
  Trash2, // Added Lock and Trash2 imports
} from "lucide-react";
...
export default function Reviews() {
  ...
  const handleSelectProduct = async (product) => {
    ...
    try {
      const res = await api.get(
        `/store/${activeStore._id}/reviews?productId=${product._id}`,
      );
      setReviews(res.data || []);
    } catch (error) {
  ...
  const submitDispute = async (e) => {
    ...
    try {
      ...
      setReviews(
        reviews.map((r) =>
          r._id === disputeData.reviewId ? res.data.data : r,
        ),
      );
  ...
  const handleReply = async (id) => {
    ...
    try {
      const res = await api.post(
        `/store/${activeStore._id}/reviews/${id}/reply`,
        { reply: replyText[id] },
      );
                ...
                <div className="flex justify-between items-center pt-4 border-t border-white/5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-bold">
                      {product.stats?.avgRating?.toFixed(1) || 0}
                    </span>
                  </div>
  ...
  return (
    <div className="p-4 md:p-8 relative z-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between mb-8 bg-white/[0.02] p-5 rounded-2xl border border-white/10 backdrop-blur-md gap-4">
        <div>
          <h1 className="text-xl md:text-3xl font-extrabold text-white mb-1 line-clamp-2">
      ...
      {loading ? (
        ...
        <div className="space-y-4 md:space-y-6">
          {reviews.map((review) => (
            <div
              key={review._id}
              className={`bg-white/[0.02] border border-white/10 p-4 md:p-6 rounded-2xl backdrop-blur-xl relative group shadow-lg ${review.isLocked ? "opacity-80" : ""}`}
            >
              <div className="flex flex-col sm:flex-row justify-between items-start mb-4 gap-4">
                ...
                <div className="flex gap-2 bg-black/40 p-1.5 rounded-lg border border-white/5 opacity-100 md:opacity-50 md:group-hover:opacity-100 transition-opacity w-full sm:w-auto justify-end">
                  ...
                  <span className="px-3 py-1 text-xs font-bold text-gray-400">
                    Rejected by Admin
                  </span>
                  ) : (
                  <button
              ...
              {review.merchantReply ? (
                ...
                <div className="flex gap-2 mt-4">
                  <input
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white focus:border-cyan-400"
                  />
                  <button
